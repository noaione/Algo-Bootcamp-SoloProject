...
if [ -z "$1" ]; then
    echo "Please provide project to compile, example: tools/compile.sh WarungBude"
    exit 1
fi

if [ "$1" != "" ]; then
    if [ "$1" == "WarungBude" ]; then
        echo "Will compile WarungBude project"
    else
...
ALL_MODULES=`ls $1/src/modules/*.cpp | tr '\n' ' '`
OUTPUT_FILE="build/$1_${BUILD_FOR}"

echo "[${TRAVIS_OS_NAME}_${BUILD_FOR}] Building..."
echo "[${TRAVIS_OS_NAME}_${BUILD_FOR}] Executing: ${CXX} $1/src/main.cpp ${ALL_MODULES} -o ${OUTPUT_FILE} -O2 -s"
$CXX $1/src/main.cpp $ALL_MODULES -o $OUTPUT_FILE $EXTRA -O2 -s
